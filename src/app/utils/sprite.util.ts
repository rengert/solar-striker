...
export function hit(spriteA: ObjectModelType, spriteB: ObjectModelType): boolean {
  ...
  if (itemHit) {
    if (spriteA.energy !== undefined && !isProtectedCollectable(spriteA, spriteB) && !spriteA.isShielded) {
      spriteA.energy -= spriteB.power;
      if (spriteA.energy <= 0) {
        spriteA.explode();
      }
    }
    if (spriteB.energy !== undefined && !isProtectedCollectable(spriteB, spriteA) && !spriteB.isShielded) {
